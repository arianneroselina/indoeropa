...
export default function Shipment({ variant = "default" }) {
	...
	const fromCountryItem = useMemo(
		() => countries.find((c) => c.id === fromCountry) ?? null,
		[countries, fromCountry],
	);

	const toCountryItem = useMemo(
		() => countries.find((c) => c.id === toCountry) ?? null,
		[countries, toCountry],
	);

	const fromLabel = fromCountryItem?.name || "";
	const toLabel = toCountryItem?.name || "";

	// =========================================================
	// Selected package type
	// =========================================================
	const selectedPackageType = useMemo(() => {
		return packageTypes.find((p) => p.id === selectedPackageTypeId) ?? null;
	...
	const recommendedLabel = useMemo(() => {
		...
		return (
			packageTypes.find((p) => p.id === recommendedPackageTypeId)
				?.label ?? recommendedPackageTypeId
		);
	}, [recommendedPackageTypeId, packageTypes]);
	...
	const scrollToDims = () => {
		requestAnimationFrame(() => {
			dimsRef.current?.scrollIntoView({
				behavior: "smooth",
				block: "start",
			});
		});
											...
											<div className="rounded-xl border bg-green-50 px-4 py-3">
												...
												<div className="mt-1 text-xs text-green-900/80 leading-relaxed">
													<span className="font-medium">
														{feedbackItem.fromLabel}{" "}
														→ {feedbackItem.toLabel}
													</span>
													{" • "}
													{formatDateToDDMMYYYY(
