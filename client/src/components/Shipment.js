...
export default function Shipment({ variant = "default" }) {
  ...
  const recommendedLabel = useMemo(() => {
    ...
    return (
      PACKAGE_TYPES.find((p) => p.id === recommendedPackageTypeId)?.label ??
  ...
  const fromLabel =
    COUNTRIES.find((c) => c.id === fromCountry)?.name ?? fromCountry;
                ...
                {needsVolumePackage && (
                  <div className="hidden sm:block mb-4 rounded-xl border bg-amber-50 px-4 py-3">
                    ...
                    <div className="subtext text-xs text-secondary/80 mt-1">
                      Your dimensions are large for the entered weight.
                      Recommended:{" "}
                  ...
                  <div className="space-y-4">
                    ...
                    {progressStep >= 2 && !isDocument && !isHat && (
                      ...
                      >
                        ...
                        <div
                          className={!dimensionAccepted ? "animate-pulse" : ""}
                        >
                          <DimensionsBlock
                            lengthCm={lengthCm}
