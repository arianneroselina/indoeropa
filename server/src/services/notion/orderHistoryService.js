...
function buildOrderProperties({
    ...
    return {
        ...
        "Selected DHL Addon": notionSelect(selectedDhlAddon),
...
export async function saveOrderHistory({ body, files }) {
    ...
    const paymentProof = files?.find((file) => file.fieldname === "paymentProof");
    ...
    for (const shipment of parsedShipments) {
        ...
        if (invoiceProofFiles.length > 0) {
            shipmentProperties["Invoice Proof"] = notionFiles(invoiceProofFiles);
        }
